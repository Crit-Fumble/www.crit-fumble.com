...
const ButtonBlock = ({title, url, target = '', options = 'height=1080, width=1152'}: {title: string, url: string, target?: string, options?: string}) => {
  ...
  return (<>
    ...
    <div className="flex flex-row gap-2">
      <button className={ DND_5E.TW_CLASSES.BUTTON + " text-nowrap" } onClick={getOpenWindowHandler(url, `popout-${target}`, options)}>
        Open Popout
      </button>
      <a className={ DND_5E.TW_CLASSES.LINK + " text-nowrap" } href={url} target={`tab-${target}`}>
        Open Tab
      </a>
    </div>
  </>);
