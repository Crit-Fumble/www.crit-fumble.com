    ...
    <div className="flex flex-col align-middle items-center">
      ...
      {parentParty?.name && (
        <div className="border-t-2 p-6">
          <span><a className={DND_5E.TW_CLASSES.LINK} href={`/play/dnd5e/${parentParty?.slug}`}>View {parentParty?.name}</a></span>
        ...
        {characters?.filter((character: any) => character?.party?.id === party?.id)?.map(
          (character: any) => (
            <div key={character?.slug}>
              <p>
                <a className={DND_5E.TW_CLASSES.LINK} key={character.id} href={`/play/dnd5e/${character?.party?.slug}/${character?.slug}`}>
                  View {character.name}
                </a>
              </p>
            </div>
        ...
        {subParties?.map((subParty: any) => (
          <div key={subParty?.slug} className="p-2">
            <div className="flex flex-col align-middle items-center p-4 gap-4">
              <a className={DND_5E.TW_CLASSES.LINK} key={subParty.id} href={`/play/dnd5e/${subParty?.slug}`}>
                View {subParty?.name}
              </a>
      ...
      <div className="flex flex-grid flex-wrap justify-middle align-middle items-middle gap-2 text-center">
        {roll20Id && <a className={DND_5E.TW_CLASSES.LINK}  href={`https://app.roll20.net/editor/setcampaign/${roll20Id}?desiredrole=player`} target="_blank">Open Roll20 Campaign</a>}
        {dndBeyondId && <a className={DND_5E.TW_CLASSES.LINK}  href={`https://www.dndbeyond.com/campaigns/${dndBeyondId}`} target="_blank">Open D&D Beyond Campaign</a>}
        {world?.url && <a className={DND_5E.TW_CLASSES.LINK} href={world.url} target="_blank">Open {world.title} World Anvil</a>}
        {voiceChannelId && <a className={DND_5E.TW_CLASSES.LINK}  href={`https://discord.com/channels/${discordServerId}/${voiceChannelId}`} target="_blank">
          Open Discord Voice
        </a>}
        {sideChatThreadId && <a className={DND_5E.TW_CLASSES.LINK}  href={`https://discord.com/channels/${discordServerId}/${sideChatThreadId}`} target="_blank">
