import prisma, { withDb } from '../DatabaseService';
import { randomUUID } from 'crypto';
import type { Character } from 'data/models/Character/Character.d';
  ...
  return await withDb(async () => {
    try {
      ...
      const response = await prisma.character.findMany({
        ...
        // Make sure we only select columns that exist in the database
        select: {
          id: true,
          name: true,
          player: true,
          ...
          title: true,
          dnd_beyond_id: true
          // Removed gameSystemId which doesn't exist in the database
        }
      });
